...
axios.interceptors.response.use(
    ...
    async error => {
        ...
        if (error.response.status === 403 && !originalRequest._retry) {
            ...
            try {
                const refreshResponse = await axios.post(`${apiUrl}/api/refresh`, {}, { withCredentials: true });
...
export async function fetchJewelImages(jewelId) {
    try {
        const response = await axios.get(`${API_BASE_URL}/jewels/${jewelId}`);
        const jewel = response.data;

        const imageNames = jewel.imageNames;
        var images = [];
        for (const imageName of imageNames) {
            const imageUrl = `${API_BASE_URL}/jewels/image/${imageName}`;
            const imageResponse = await axios.get(imageUrl, { responseType: 'blob' }); 
            images.push(URL.createObjectURL(imageResponse.data));
        }
        return images;
    } catch (error) {
        console.log("Error getting jewel images")
        throw error;
    }
}